...
from .library_tello import Tello
...
class TelloNode():
    def __init__(self, node):
        ...
        self.node.declare_parameter('tello_ip', '192.168.0.102')
        ...
        self.tello = Tello(host='192.168.0.100',local_port=9010,state_port=8891)
        self.tello.connect()
    ...
    def setup_publishers(self):
        ...
        self.pub_pose_imu = self.node.create_publisher(PoseStamped, 'pose_imu',10)
    ...
    def setup_subscribers(self):
        ...
        self.sub_control = self.node.create_subscription(Twist, 'control', self.cb_control, 10)
        self.sub_flip = self.node.create_subscription(String, 'flip', self.cb_flip, 10)
        ...
        def status_odom():
            while True:
                ...
                if self.pub_imu.get_subscription_count() > 0:
                    ...
                    pose.header.frame_id = self.tf_drone
                    pose.pose.orientation.x = q[0]
                    pose.pose.orientation.y = q[1]
                    pose.pose.orientation.z = q[2]
                    pose.pose.orientation.w = q[3]
                    self.pub_pose_imu.publish(pose)
